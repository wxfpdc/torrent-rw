...
class Magnet implements TorrentInterface {
	...
	public function create($hash, $name = null, $trackers = array()) {		
	...
	public function parse($magnet_uri) {
		...
		$this->parsed = array();
		$for_parse = str_replace("magnet:?", "", $magnet_uri); //trim off the first part so the rest can be parsed as a querystring
		$for_parse = str_replace("&tr=", "&tr[]=", $for_parse); //convert &tr= to &tr[]= so that parse_str returns a populated array with an element for each recurring tr= param instead of flattening them
